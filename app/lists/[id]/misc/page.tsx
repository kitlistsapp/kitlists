            ...
            {miscItems.map(item => (
              <div key={item.id}>
                <label className="flex items-center gap-3 px-4 py-2.5 rounded-lg hover:bg-zinc-800 transition-colors cursor-pointer group">
                  <div
                    className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 border transition-colors ${ selected.includes(item.id) ? 'bg-orange-400 border-orange-400' : 'border-zinc-600 group-hover:border-zinc-400' }`}
                  ...
                  <div className="ml-12 mt-1 mb-2 flex gap-2 items-start">
                    <div className="flex gap-1">
                      ...
                      <button onClick={() => setMiscSource(prev => ({ ...prev, [item.id]: 'dop_owned' }))}
                        className={`px-2.5 py-1 rounded text-xs font-medium transition-colors ${ miscSource[item.id] === 'dop_owned' ? 'bg-orange-400 text-black' : 'bg-zinc-800 text-zinc-500 hover:bg-zinc-700' }`}>
                        DOP owned
                      </button>
                    </div>
                    <input
                      ...
                      className="flex-1 bg-zinc-800 border border-zinc-700 text-white rounded px-3 py-1 text-xs focus:outline-none focus:border-orange-400"
                    />
                  </div>
                )}
              </div>
          ...
          <div className="border-t border-zinc-800 pt-6">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-zinc-800 border border-zinc-700 text-white rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-orange-400"
              />
