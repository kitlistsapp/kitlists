...
export default function HeadTripodPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    await supabase.from('list_items').delete().eq('list_id', listId).eq('section', 'head_tripod')
    const rows = entries.filter(e => e.itemId).map((e, i) => ({
      list_id: listId, owner_id: userId, section: 'head_tripod',
    ...
    if (notesId) {
      ...
      const { data: newNote } = await supabase.from('list_section_notes').insert({ list_id: listId, owner_id: userId, section: 'head_tripod', notes: sectionNotes }).select().single()
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <h2 className="text-2xl font-bold mb-2">Head & Tripod</h2>
              ...
              <div className="flex gap-2 items-center">
                <div className="flex-1">
                  <SearchablePicker items={allItems} value={entry.itemId}
                    ...
                    placeholder="Search heads & tripods..." />
                </div>
