...
export default function GripPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const save = async () => {
    ...
    setSaving(true)
    await supabase.from('list_items').delete().eq('list_id', lid).eq('section', 'grip')
    const rows = ents.filter(e => e.itemId).map((e, i) => ({
      list_id: lid, owner_id: uid, section: 'grip',
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        <h2 className="text-2xl font-bold mb-2">Gimbals</h2>
        <p className="text-zinc-500 text-sm mb-6">Gimbals and controllers</p>
              ...
              <div className="flex gap-2 items-center min-w-0">
                <div className="flex-1">
                  <SearchablePicker items={allItems} value={entry.itemId}
                    ...
                    placeholder="Search gimbals and controllers..." />
                </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 mt-4">
          ...
          <textarea value={sectionNotes} onChange={e => setSectionNotes(e.target.value)}
            placeholder="Any notes about gimbals or controllers..."
            rows={3}
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-zinc-800">
          <a href={"/lists/" + listId + "/head-tripod"}
          ...
          <button onClick={async () => { await save(); window.location.href = "/lists/" + listId + "/vtr"; }}
              disabled={saving}
