...
type LensBrowserProps = {
  listId: string
  onSave?: (selected: SelectedLens[]) => void
}
...
export default function LensBrowser({ listId, onSave }: LensBrowserProps) {
    ...
    <div className="flex flex-col h-full bg-black text-white select-none">
      ...
      <div className="flex-none border-t border-zinc-800 px-4 py-3 bg-black flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => onSave?.(Array.from(kit.values()))} disabled={kitCount === 0} className={`px-4 py-1.5 rounded text-sm font-medium transition-all ${kitCount > 0 ? 'bg-orange-400 text-black hover:bg-orange-300' : 'bg-zinc-800 text-zinc-600 cursor-not-allowed'}`}>
            Save to list
          </button>
        </div>
      </div>
